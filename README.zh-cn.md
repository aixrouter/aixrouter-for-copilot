...
- AIXRouter 模型列表默认从 `/openai/v1/models` 动态读取，也可以用设置固定展示指定模型。
...
AIXRouter 会把 `aixrouter.baseUrl` 视为网关根地址。默认 `https://api.aixrouter.com` 下，请求会这样路由：
...
| 其他 OpenAI 兼容模型 | `https://api.aixrouter.com/openai/v1/chat/completions` |

模型列表从 `https://api.aixrouter.com/openai/v1/models` 加载。

## 设置项

| 设置项 | 默认值 | 说明 |
| --- | --- | --- |
| `aixrouter.baseUrl` | `https://api.aixrouter.com` | AIXRouter 网关 Base URL，插件会按模型自动拼接具体协议路径 |
...
`{baseUrl}/openai/v1/models` 是模型列表和模型能力的权威来源。为了让 Copilot 模型选择器显示更完整，建议每个模型返回能力、上下文和价格字段：
...
认证方式为 `Authorization: Bearer <your-api-key>`。本扩展从 `{baseUrl}/openai/v1/models` 加载模型，并按模型类型把聊天请求路由到 `/claude/v1`、`/gemini/v1beta`、`/vertexai/v1` 或 `/openai/v1`。公开模型页只用于补充元数据：当 Base URL 属于 `aixrouter.com` 时访问 `https://www.aixrouter.com/models`，属于 `agilerouter.com` 时访问 `https://www.agilerouter.com/models`；其他域名不会访问这两个页面。
